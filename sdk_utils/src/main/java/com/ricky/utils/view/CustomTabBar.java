...
import com.ricky.utils.R;
import com.ricky.utils.utils.CustomFragmentTabHost;
import com.ricky.utils.utils.TintUtil;
...
public class CustomTabBar extends LinearLayout {
    ...
    public CustomTabBar(Context context, AttributeSet attrs) {
        ...
        TypedArray attributes = context.obtainStyledAttributes(attrs, R.styleable.CustomTabBar);
        if (attributes != null) {
            //图片宽度
            imgWidth = attributes.getDimension(R.styleable.CustomTabBar_tab_img_width, 0);
            //图片高度
            imgHeight = attributes.getDimension(R.styleable.CustomTabBar_tab_img_height, 0);
            //文字尺寸
            fontSize = attributes.getDimension(R.styleable.CustomTabBar_tab_font_size, 14);
            //上边距
            paddingTop = attributes.getDimension(R.styleable.CustomTabBar_tab_padding_top, dp2px(2));
            //图片文字间隔
            fontImgPadding = attributes.getDimension(R.styleable.CustomTabBar_tab_img_font_padding, dp2px(3));
            //下边距
            paddingBottom = attributes.getDimension(R.styleable.CustomTabBar_tab_padding_bottom, dp2px(5));
            //分割线高度
            dividerHeight = attributes.getDimension(R.styleable.CustomTabBar_tab_divider_height, dp2px(1));
            //是否显示分割线
            isShowDivider = attributes.getBoolean(R.styleable.CustomTabBar_tab_isshow_divider, false);
            //选中的颜色
            selectColor = attributes.getColor(R.styleable.CustomTabBar_tab_selected_color, Color.parseColor("#626262"));
            //未选中的颜色
            unSelectColor = attributes.getColor(R.styleable.CustomTabBar_tab_unselected_color, Color.parseColor("#F1453B"));
            //BottomTabBar的整体背景
            tabBarBackgroundColor = attributes.getColor(R.styleable.CustomTabBar_tab_bar_background, Color.parseColor("#FFFFFF"));
            //分割线背景
            dividerBackgroundColor = attributes.getColor(R.styleable.CustomTabBar_tab_divider_background, Color.parseColor("#CCCCCC"));
     ...
     * @return
     */
    public CustomTabBar init(FragmentManager manager) {
    ...
    public CustomTabBar setOnTabChangeListener(OnTabChangeListener listener) {
        if (listener != null) {
     ...
     * @return
     */
    public CustomTabBar addTabItem(String name, int imgId, Class fragmentClass) {
        return addTabItem(name, ContextCompat.getDrawable(context, imgId), fragmentClass);
    }

    public CustomTabBar addTabItem(String name, int imgIdSelect, int imgIdUnSelect, Class fragmentClass) {
     ...
     * @return
     */
    public CustomTabBar addTabItem(final String name, Drawable drawable, Class fragmentClass) {
    ...
    public CustomTabBar addTabItem(final String name, Drawable drawableSelect, Drawable drawableUnSelect, Class fragmentClass) {
        tabIdList.add(TextUtils.isEmpty(name) ? fragmentClass.getName() : name);
        mTabHost.addTab(mTabHost.newTabSpec(TextUtils.isEmpty(name) ? fragmentClass.getName() : name)
     ...
     * @return
     */
    public CustomTabBar setImgSize(float width, float height) {
        this.imgWidth = width;
     ...
     * @return
     */
    public CustomTabBar setFontSize(float textSize) {
     ...
     * @return
     */
    public CustomTabBar setTabPadding(float top, float middle, float bottom) {
        this.paddingTop = top;
     ...
     * @return
     */
    public CustomTabBar setChangeColor(@ColorInt int selectColor, @ColorInt int unSelectColor) {
     ...
     * @return
     */
    public CustomTabBar setTabBarBackgroundColor(@ColorInt int color) {
     ...
     * @return
     */
    public CustomTabBar setTabBarBackgroundResource(@DrawableRes int resid) {
...
//    }
     ...
     * @return
     */
    public CustomTabBar isShowDivider(boolean isShowDivider) {
     ...
     * @return
     */
    public CustomTabBar setDividerHeight(float height) {
     ...
     * @return
     */
    public CustomTabBar setDividerColor(@ColorInt int color) {
